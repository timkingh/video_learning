...
for jj = 0:7
    for kk = 0:7
        ...
        x1_path = ".\log_5\x1_r48_A" + num2str(jj) + file_name;
        y1_path = ".\log_5\y1_r48_A" + num2str(jj) + file_name;
        fid_out_0 = fopen(x1_path, 'w');
        fid_out_1 = fopen(y1_path, 'w');
        fprintf("jj %d kk %d\n", jj, kk);
        [fid_dering0, msg] = fopen('.\log_5\jpg_dec_coef_test_r43_v2_hisi_r39_qt10_dering0.txt', 'r');
        if fid_dering0 == -1
            disp(msg);
            return;
        end

        [fid_dering1, msg] = fopen('.\log_5\jpg_dec_coef_test_r43_v2_hisi_r39_qt10_dering1.txt', 'r');
        if fid_dering1 == -1
            disp(msg);
            return;
        end
